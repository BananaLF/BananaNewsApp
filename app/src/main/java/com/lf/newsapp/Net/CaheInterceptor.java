...
public class CaheInterceptor implements Interceptor {
    ...
    public Response intercept(Chain chain) throws IOException {
        ...
        /**
         * 添加http缓存机制
         *  NetworkUtil.isNetworkAvailable(context)判断网络是否可用
         *  return 返回True 则从缓存读60s内的缓存数据
         *  return 返回False 则从缓存读3天内的缓存数据
         */
        if (NetworkUtil.isNetworkAvailable(context) && NetworkUtil.getNetState(context) == NetworkUtil.NET_CNNT_BAIDU_OK) {
